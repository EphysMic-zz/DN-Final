/*
Если прозрачный материал может нарисовать поверх сцены, он не может изменить то, что было нарисовано под ним. 
		...
		Pass {
			...
			fixed4 frag (v2f_vct i) : COLOR
			{
				...
				*/
				...
				half2 distortion = half2 (tex2D(_NoiseTex, i.worldPos.xy / _Scale + half2(sinT, 0) ).r - 0.5,
																		tex2D(_NoiseTex, i.worldPos.xy / _Scale + half2(0, sinT) ).r - 0.5
				);
